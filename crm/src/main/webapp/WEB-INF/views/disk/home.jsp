                ...
                <div class="box-header with-border">
                    ...
                    <div class="box-tools pull-right">
                    	<c:if test="${not empty disk}">
	                        <button id="back" class="btn btn-default btn-sm"><i class="fa fa-arrow-left"></i> 返回上一级</button>
	                        <%-- <a href="/disk/home?pid=${disk.pId == 0 ? '' :'disk.pId'}" class="btn btn-default btn-sm"><i class="fa fa-arrow-left"></i> 返回上一级</a> --%>
                    	</c:if>
                    	<c:choose>
                    		...
                    		<c:otherwise>
		                    	<div id="upload"><i class="fa fa-upload"></i>上传文件</div>
                    				...
                    				<td colspan="4">
                    					<c:choose>
                    						<c:when test="${disk.name.endsWith('.pdf') or disk.name.endsWith('.jpg') or disk.name.endsWith('.gif') or disk.name.endsWith('.jpeg')}">
				                    			<a href="/disk/download?id=${disk.id}" target="_block" class="btn btn-default btn-sm"><i class="fa fa-search"></i>预览</a>
				                    			<a href="/disk/download?id=${disk.id}&fileName=${disk.name}" class="btn btn-info btn-sm"><i class="fa fa-download"></i> 下载</a>          
	                   						</c:when>
	                   						<c:otherwise>
				                    			<a href="/disk/download?id=${disk.id}&fileName=${disk.name}" class="btn btn-info btn-sm"><i class="fa fa-download"></i> 下载</a>          
		                    	...
		                    	<c:forEach items="${diskList}" var="disk">
			                        	...
			                        	<c:choose>
			                        		<c:when test="${disk.type == 'dir' }">
					                            <td class="tr" rel="${disk.id }" width="50" class="file_icon"><i class="fa fa-folder-o"></i></td>
			                        		</c:when>
			                        		<c:otherwise>
					                            <td class="tr" rel="${disk.id }" width="50" class="file_icon"><i class="fa fa-file-o"></i></td>
			                        		</c:otherwise>
			                        	</c:choose>
			                            <td class="tr" rel="${disk.id }">${disk.name}</td>
			                            <td class="tr" rel="${disk.id }"><fmt:formatDate value="${disk.updateTime}" pattern="yyyy年MM月dd日 "/></td>
			                            <td class="tr" rel="${disk.id }" width="100">${disk.fileSize}</td>
			                            <td width="150">
			                                <div class="btn-group">
			                                    ...
			                                    <ul class="dropdown-menu">
			                                        ...
			                                        <li><a href="/disk/del?id=${disk.id}">删除</a></li>
			                                    </ul>
			                                </div>
			                            </td>
			                       </tr>
 	...
 	$(function(){
 		...
 		$("#addFolderBtn").click(function(){
 			layer.prompt({title:"请输入文件夹名称"},function(text,index){
 				layer.close(index);
 				$.post("/disk/add",{"pid":pid,"name":text}).done(function(data) {
 		...
 		uploader.on('uploadStart',function(file) {
 			loadIndex = layer.load(2);//上传提示用的小圈圈
 		});
 		...
 		$(".rename").click(function(){
 			...
 			layer.prompt({title:"请输入新的文件名"},function(text,index){
 				layer.close(index);
 				$.post("/disk/rename",{"diskId":diskId,"rename":text}).done(function(data){
 					if(data.state == 'success') {
 						layer.msg("修改成功");
 						history.go(0)
 					}
