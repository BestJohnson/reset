                    ...
                    <ul class="todo-list">
                    	<c:forEach items="${taskList}" var="task">
                    		<li class="${task.status == 1 ? 'done' : '' }">
	                            <input type="checkbox" class="changeStatus" rel="${task.id}" ${task.status == 1 ? 'checked' : '' }>
	                            <span class="text">${task.title}</span>
	                            <small class="label ${task.overTime ? 'label-danger' : 'label-success' }"><i class="fa fa-clock-o"></i> ${task.finishTime}</small>
	                            <div class="tools">
	                                <a href="/task/edit?id=${task.id }"class="fa fa-edit"></a>
	                                <a href="/task/del?id=${task.id }" class="fa fa-trash-o"></a>
	                            </div>
	                        </li>
                    	</c:forEach>
...
<script>
	$(function(){
		$(".changeStatus").click(function() {
			...
			var checked = $(this)[0].checked;//对象$(this)[0]调用checked事件
			...
			$.get("/task/updatestatus",{"taskId":taskId, "status":status}, function(data){
